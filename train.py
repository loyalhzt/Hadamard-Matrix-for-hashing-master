...
def main(args):
    ...
    print(true_hash)
    Hash_center = torch.load(true_hash)  # true_hash是一个训练好的模型，在train时调用
    ...
    for epoch in range(total_epoch):
        ...
        if epoch % 5 == 0:
            ...
            if MAP > best_MAP:
                ...
                dir_name = 'data/' + file_dir + '/' + 'models/' + str(args.hash_bit) + 'bit_' + str(epoch) + 'e_' + str(
                    "{:.4g}".format(MAP)) + '_' + args.model_type + '.pkl'
                if args.data_name == 'test':
                    torch.save(model, "../model/test.pkl")
                    print('save model in: %s' % "../model/test.pkl")
                else:
...
def train(model, args, train_loader, criterion, Hash_center, optimizer, epoch, two_loss_epoch):
    ...
    for i, (input, label) in enumerate(train_loader):
        ...
        if i % 100 == 0:
            ...
            epoch, lr, iter_num, (end_time1 - start_time), loss))
